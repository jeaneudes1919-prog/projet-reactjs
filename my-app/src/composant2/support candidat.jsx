...
export default function SupportCandidat() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch("/api/support-candidat/envoyer", {
        method: "POST",
  ...
  const fetchMessages = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await fetch("/api/support-candidat/messages", {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
      setMessages(data);
  ...
  return (
    <div className="max-w-10xl mx-auto px-20 py-10 rounded-3xl shadow-lg border border-gray-200 bg-white dark:bg-gray-900 dark:border-gray-700">
